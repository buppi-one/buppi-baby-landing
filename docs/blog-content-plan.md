# Blog — plano de conteúdo

Roadmap de artigos para os primeiros 3 meses de blog. Use como checklist —
marque `[x]` quando publicar, anote o slug final usado e a data.

> Padrões de escrita, formato de FAQ e referências, e workflow de criação:
> ver [`blog.md`](./blog.md).

---

## Estratégia

Três tipos de conteúdo, na proporção:

| Tipo | % | Por quê |
|---|---|---|
| **Evergreen guides** | 50% | Resolvem dúvidas eternas → SEO orgânico durável |
| **Long-tail por idade** | 35% | Cada "sono bebê 3 meses" é uma busca real, escala linear |
| **Buppi-feature** | 15% | Mostra valor do app no contexto de uma necessidade (não é venda direta) |

**Velocidade**: ~2 artigos publicados por semana (~8/mês). Em 3 meses = ~24 artigos
de conteúdo principal + ~10 de long-tail = base sólida (~34 publicados).

**Tradução**: pt-BR + en sempre. ES/FR podem ficar 1–2 semanas atrás pra
otimizar tempo. Em conteúdo regional (legislação BR, SUS, etc.) só pt-BR.

---

## Wave 1 — primeiros 30 dias

Os **must-haves de SEO**. Alta busca, evergreen, dúvidas universais. Publicar
nessa ordem (cobre os principais "primeiros 6 meses" do bebê).

| # | Status | Categoria | Título | Slug | FAQ |
|---|---|---|---|---|---|
| 1 | ✅ 2026-05-06 (4 idiomas) | sleep | Quanto tempo um bebê deve dormir por idade (recém-nascido a 2 anos) | `quanto-tempo-bebe-deve-dormir-por-idade` | ✅ |
| 2 | ✅ 2026-05-07 (4 idiomas) | feeding | Amamentação nos primeiros dias: o que esperar | `amamentacao-primeiros-dias` | ✅ |
| 3 | ✅ 2026-05-07 (4 idiomas) | expecting-and-new-parents | Como se preparar para a chegada do bebê: checklist completo | `checklist-chegada-do-bebe` | ✅ |
| 4 | ✅ 2026-05-07 (4 idiomas) | sleep | Janelas de sono (wake windows): a tabela por idade | `janelas-de-sono-por-idade` | ✅ |
| 5 | ✅ 2026-05-07 (4 idiomas) | feeding | Introdução alimentar aos 6 meses: BLW vs papinha tradicional | `introducao-alimentar-6-meses-blw-vs-papinha` | ✅ |
| 6 | ✅ 2026-05-07 (4 idiomas) | health | Calendário de vacinas do bebê no primeiro ano (Brasil) | `calendario-vacinas-primeiro-ano` | ✅ |
| 7 | ✅ 2026-05-07 (4 idiomas) | development | Marcos do desenvolvimento: o que esperar mês a mês (0-12) | `marcos-desenvolvimento-mes-a-mes` | ✅ |
| 8 | ✅ 2026-05-07 (4 idiomas) | sleep | Regressão do sono dos 4 meses: por que acontece e como passar | `regressao-sono-4-meses` | ✅ |
| 9 | ❌ removido — duplicava `como-acalmar-bebe-colica` (que já cobre identificação + tratamento) | expecting-and-new-parents | Sinais de que o bebê está com cólica (e o que fazer) | `sinais-de-colica-no-bebe` | — |
| 10 | ✅ 2026-05-07 (4 idiomas) | feeding | Como saber se o bebê está mamando o suficiente | `bebe-mamando-o-suficiente` | ✅ |

---

## Wave 2 — dias 31-60

Aprofundando cada categoria, começando a explorar long-tail.

| # | Status | Categoria | Título | Slug |
|---|---|---|---|---|
| 11 | ✅ 2026-05-11 (4 idiomas) | sleep | Soneca do bebê: quantas, quanto tempo, em que horários | `soneca-do-bebe-guia` |
| 12 | ✅ 2026-05-16 (4 idiomas) | sleep | Como criar uma rotina de sono que funciona | `como-criar-rotina-de-sono` |
| 13 | ✅ 2026-06-09 (4 idiomas) | feeding | Tabela de quantidade de fórmula por idade | `quantidade-de-formula-por-idade` |
| 14 | ✅ 2026-06-11 (4 idiomas) | feeding | Cluster feeding: por que o bebê quer mamar o tempo todo | `cluster-feeding-explicado` |
| 15 | ✅ 2026-06-23 (4 idiomas) | development | Tummy time: por que importa, como fazer e por quanto tempo | `tummy-time-guia-completo` |
| 16 | ✅ 2026-06-26 (4 idiomas) | expecting-and-new-parents | Os primeiros 30 dias com o recém-nascido: guia de sobrevivência | `primeiros-30-dias-recem-nascido` |
| 17 | ✅ 2026-06-30 (4 idiomas) | health | Febre no bebê: quando se preocupar e quando ligar pro pediatra | `febre-no-bebe-quando-se-preocupar` |
| 18 | ✅ 2026-07-06 (4 idiomas) | sleep | Sono compartilhado (cama compartilhada): riscos e diretrizes | `cama-compartilhada-riscos-diretrizes` |
| 19 | ✅ 2026-07-07 (4 idiomas) | feeding | Refluxo no bebê: sinais, manejo e quando virar refluxo patológico | `refluxo-no-bebe-sinais-manejo` |
| 20 | ✅ 2026-07-13 (4 idiomas) | expecting-and-new-parents | Como escolher o pediatra do seu filho | `como-escolher-pediatra` |

---

## Wave 3 — dias 61-90

Onde entram os **artigos de feature do Buppi** + mais long-tail.

| # | Status | Categoria | Título | Slug | Tipo |
|---|---|---|---|---|---|
| 21 | ⏭️ pulado — sobre o app (a pedido) | sleep | Como usar o Buppi para identificar padrões de sono do seu bebê | `usar-buppi-padroes-de-sono` | feature |
| 22 | ⏭️ pulado — sobre o app (a pedido) | feeding | Como o Buppi facilita o registro de mamadas e dá insights | `usar-buppi-registro-mamadas` | feature |
| 23 | ⏭️ pulado — sobre o app (a pedido) | health | Como organizar vacinas e consultas no Buppi | `usar-buppi-vacinas-e-consultas` | feature |
| 24 | ✅ 2026-07-20 (4 idiomas) | sleep | Sono do bebê com 2 meses: cronograma e o que esperar | `sono-bebe-2-meses` | long-tail |
| 25 | ✅ 2026-07-22 (4 idiomas) | sleep | Sono do bebê com 3 meses: cronograma e o que esperar | `sono-bebe-3-meses` | long-tail |
| 26 | ✅ 2026-07-23 (4 idiomas) | sleep | Sono do bebê com 6 meses: cronograma e o que esperar | `sono-bebe-6-meses` | long-tail |
| 27 | ✅ 2026-07-23 (4 idiomas) | development | Marcos do desenvolvimento aos 6 meses | `marcos-desenvolvimento-6-meses` | long-tail |
| 28 | ✅ 2026-07-24 (4 idiomas) | development | Marcos do desenvolvimento aos 12 meses | `marcos-desenvolvimento-12-meses` | long-tail |
| 29 | ✅ 2026-07-24 (4 idiomas) | expecting-and-new-parents | Saúde mental no pós-parto: depressão e baby blues | `saude-mental-pos-parto` | evergreen |
| 30 | ⏭️ pulado — sobre o app (a pedido) | news | Buppi 1.0: o que mudou e o que vem por aí | `buppi-1-0-novidades` | news |

---

## Conteúdo regional (só pt-BR — sem tradução)

Específicos do Brasil. Lançar conforme demanda, **não criar arquivos `en.mdx`/`es.mdx`/`fr.mdx`**.

| Status | Categoria | Título | Slug |
|---|---|---|---|
| ✅ 2026-08-13 (pt-BR) | expecting-and-new-parents | Licença-maternidade e paternidade no Brasil: tudo o que você precisa saber | `licenca-maternidade-paternidade-brasil` |
| ✅ 2026-09-04 (4 idiomas) | health | SUS x rede privada: como funciona o pré-natal no Brasil | `pre-natal-sus-vs-privado` |
| ⬜ | feeding | Bancos de leite humano no Brasil: como doar e como receber | `bancos-de-leite-humano-brasil` |
| ⬜ | expecting-and-new-parents | Documentos do recém-nascido: registro civil, CPF, plano de saúde | `documentos-do-recem-nascido` |

---

## Long-tail series — escala via SEO

Estratégia do Huckleberry: artigos curtos (~700 palavras) por **idade específica**. Cada
um captura busca tipo "sono bebê 5 semanas". Tabela de horários, dicas de transição,
link pros guias gerais. Lançar 1/semana paralelo às waves principais.

### Série A — Cronograma de sono por idade

Newborn (semanal):
- ⬜ Semana 1 — `sono-bebe-semana-1`
- ⬜ Semana 2 — `sono-bebe-semana-2`
- ⬜ Semana 3 — `sono-bebe-semana-3`
- ⬜ Semana 4 — `sono-bebe-semana-4`
- ⬜ Semana 6 — `sono-bebe-semana-6`
- ⬜ Semana 8 — `sono-bebe-semana-8`

1º ano (mensal — alguns já estão na Wave 3):
- ⬜ Mês 3 (Wave 3 #25)
- ✅ 2026-07-27 (4 idiomas) — Mês 4 — `sono-bebe-4-meses` (a regressão dos 4 meses)
- ✅ 2026-07-27 (4 idiomas) — Mês 5 — `sono-bebe-5-meses`
- ✅ 2026-07-27 (4 idiomas) — Mês 7 — `sono-bebe-7-meses`
- ✅ 2026-07-27 (4 idiomas) — Mês 8 — `sono-bebe-8-meses`
- ✅ 2026-08-05 (4 idiomas) — Mês 9 — `sono-bebe-9-meses`
- ⬜ Mês 10 — `sono-bebe-10-meses`
- ⬜ Mês 11 — `sono-bebe-11-meses`
- ⬜ Mês 12 — `sono-bebe-12-meses`

Toddler:
- ⬜ Mês 15 — `sono-bebe-15-meses`
- ⬜ Mês 18 — `sono-bebe-18-meses`
- ⬜ Mês 24 — `sono-bebe-24-meses`

### Série B — Marcos por idade

- ⬜ Mês 1 — `marcos-desenvolvimento-1-mes`
- ⬜ Mês 2 — `marcos-desenvolvimento-2-meses`
- ⬜ Mês 3 — `marcos-desenvolvimento-3-meses`
- ⬜ Mês 4 — `marcos-desenvolvimento-4-meses`
- ⬜ Mês 5 — `marcos-desenvolvimento-5-meses`
- ⬜ Mês 7 — `marcos-desenvolvimento-7-meses`
- ⬜ Mês 8 — `marcos-desenvolvimento-8-meses`
- ⬜ Mês 9 — `marcos-desenvolvimento-9-meses`
- ⬜ Mês 10 — `marcos-desenvolvimento-10-meses`
- ⬜ Mês 11 — `marcos-desenvolvimento-11-meses`
- ⬜ Mês 18 — `marcos-desenvolvimento-18-meses`
- ⬜ Mês 24 — `marcos-desenvolvimento-24-meses`

---

## Wave 4 — clusters validados pelo GSC + playbook de referência (set/2026)

Estruturada em 2026-09-23 cruzando o GSC (28d: 70k impressões, 88% na página de
fórmula, queries por idade na posição 8–10 com 0 cliques) com o playbook do
nappi.app/blog (séries por idade, artigos-tabela de referência, comparativos
"best apps", split de fórmula em artigos por mês). Ordem = potencial de clique.

**Regra anti-canibalização:** os artigos por idade respondem UMA idade a fundo
(por mamada, por dia, por peso, sinais) e linkam de volta pro pilar
`quantidade-de-formula-por-idade`, que segue sendo o hub com a tabela geral.

| # | Status | Categoria | Título | Slug | Racional |
|---|---|---|---|---|---|
| 1 | ✅ 2026-09-23 (4 idiomas) | feeding | Quantos ml de fórmula toma um bebê de 1 mês? | `formula-bebe-1-mes` | maior cluster do GSC (~800+ imp/28d reportadas, pos 8–10, 0 clique) |
| 2 | ✅ 2026-09-24 (4 idiomas) | feeding | Quantas latas de fórmula um bebê gasta por mês? (com calculadora) | `latas-de-formula-por-mes` | cluster próprio no GSC (193+ imp, pos 7–8); intenção de custo, converte |
| 3 | ✅ 2026-09-25 (4 idiomas) | feeding | Quantos ml de fórmula toma um bebê de 2 meses? | `formula-bebe-2-meses` | 2º maior cluster por idade do GSC |
| 4 | ⬜ | health | Fraldas molhadas por dia: tabela de hidratação do recém-nascido | `fraldas-molhadas-por-dia` | artigo-tabela (playbook nappi); vira carrossel table no IG; liga ao tracking de fraldas do app |
| 5 | ⬜ | feeding | Quantos ml de fórmula toma um bebê de 3 e 4 meses? | `formula-bebe-3-4-meses` | fecha o trio de idades com volume no GSC |
| 6 | ⬜ | development | Curva de crescimento do bebê: como ler peso e altura (percentis OMS) | `curva-de-crescimento-bebe` | referência evergreen; conecta ao tracking de crescimento do app |
| 7 | ⬜ | sleep | Sono do recém-nascido: semanas 1 a 4 | `sono-bebe-semana-1` (série) | preenche o buraco 0–2 meses da série de sono (nappi cobre 1–18m) |

Removido a pedido (2026-09-24): comparativo de apps e qualquer artigo sobre o próprio app — foco em conteúdo geral.

Depois da Wave 4, avaliar: série de gravidez semana a semana (nappi faz 12–40 —
29 artigos, só compensa com o funil de gestante maduro) e marcos mês a mês
(já listados na long-tail series).

---

## Cadência prática sugerida

| Mês | Wave principal | Long-tail extra | Total publicados |
|---|---|---|---|
| Mês 1 | 10 (Wave 1) | 4 (Série A — semanas) | 14 |
| Mês 2 | 10 (Wave 2) | 4 (Série B — marcos cedo) | 14 |
| Mês 3 | 10 (Wave 3) | 4 (Série A — meses 1º ano) | 14 |
| **Total 3m** | **30** | **12** | **~42** |

Em pt-BR + en. ES/FR podem rodar atrás (~2 semanas).

---

## Manutenção do plano

- Quando publicar um artigo, marcar `[x]`, anotar slug final usado e data
- Adicionar novos artigos a este plano antes de escrever (evita duplicação de
  esforço e ajuda a planejar interlinks)
- Revisar a cada trimestre: o que ranqueou bem? o que precisa ser reescrito?

Last updated: 2026-05-06
